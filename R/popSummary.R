...
#' variance (=variance of additive-by-additive epistatic deviations)
#' for all traits
#'
#' @param pop an object of \code{\link{Pop-class}}
#' @param simParam an object of class \code{\link{SimParam}}. If
...
#'
#' @details
#' With imprinting the output doesn't consider imprinting effects 
#' into breeding values of individuals.
#' If you must get sex-specific breeding values, 
#' use \code{bvSS()}, \code{bvM()} and \code{bvP()} - see above.
#'
#' @examples
#' #Create founder haplotypes
#' founderPop = quickHaplo(nInd=10, nChr=1, segSites=10)
...
#' @title Sex-specific breeding values
#'
#' @description Returns breeding values for all traits having into account
#' imprinting effects.
#'
#' @param pop an object of \code{\link{Pop-class}}
#' @param simParam an object of \code{\link{SimParam}}
...
#'
#' @export
bvSS = function(pop,simParam=NULL){
  
  if(is.null(simParam)){
    simParam = get("SP",envir=.GlobalEnv)
  }
  
  ret = bv(pop,simParam=simParam)
  ret[,1] = rep(9,nInd(pop))
  impTest = sapply(simParam$traits, FUN = function(x) .hasSlot(x,"impEff"))
  if(any(impTest)){
    retM = bvM(pop,simParam=simParam)
    retP = bvP(pop,simParam=simParam)
    sel = pop@sex %in% "F"
    if(any(sel)){
      ret[sel, impTest] = retM[sel, impTest] # for females use mat breed val
    }
    sel = pop@sex %in% "M"
    if(any(sel)){
      ret[sel, impTest] = retP[sel, impTest] # for males use pat breed val
    }
  }
  return(ret)
}

...
```
Alternatively, this can be also done using:
```{r}
calculated_gv_alt <- intercept + ((MatHaplo + PatHaplo)-1) * addEff + (PatHaplo - MatHaplo) * impEff

```

This plot shows the Genomic values using newPop function and our alternative results are the same.

```{r non.finished.plotting, eval=FALSE}
plot(calculated_gv_alt, pop@gv, abline(a = 0, b = 1)) 
